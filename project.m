...
x0 = [deg2rad(-90) 0 deg2rad(0) 0]';
% Equilibrium
xe = [deg2rad(-50) 0 deg2rad(0) 0]';
...
[t1,x_nlin] = ode45(@(t,x) Xd_nlin_func(0, 0, x(1), x(3), x(2), x(4)), [0 tfinal]', x0);
[t2,x_lin]  = ode45(@(t,x) Xd_lin_func(0, 0, x(1), x(3), x(2), x(4)), [0 tfinal]', x0-xe);
...
%     0 0 kp2 kd2];
% 
%[t3,x_ctrl_lin] = ode45(@(t,x) Xd_lin_func([1 0]*K*(x_des - x), [0 1]*K*(x_des - x), x(1), x(3), x(2), x(4)), [0 tfinal]', x0);

%% State Feedback Controller (Gain Matrix)
P = [-1 -2 -3 -4];
k = place(double(A), double(B), P);
A_new = double(A) - double(B)*k;
double(eig(A_new))
Xd_lin_new = A_new*[q1; q1d; q2; q2d] + B*[T1; T2];
Xd_lin_func_new = matlabFunction(Xd_lin_new);
% check which of the following is correct:
%[t3,x_ctrl_lin] = ode45(@(t,x) Xd_lin_func_new([1 0]*k*(x_des - x) + double(T1_init), [0 1]*k*(x_des - x) + double(T2_init), x(1), x(3), x(2), x(4)), [0 tfinal]', x0);
%[t3,x_ctrl_lin] = ode45(@(t,x) Xd_lin_func_new([1 0 0 0]*(x_des - x) + double(T1_init), [0 0 1 0]*(x_des - x) + double(T2_init), x(1), x(3), x(2), x(4)), [0 tfinal]', x0);
[t3,x_ctrl_lin] = ode45(@(t,x) Xd_lin_func_new([1 0 0 0]*(x_des - x), [0 0 1 0]*(x_des - x), x(1), x(3), x(2), x(4)), [0 tfinal]', x0);
%% Plot
figure()
%plot(t1,x_nlin(:,1));
%hold on;
